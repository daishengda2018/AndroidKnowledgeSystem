...
堆在逻辑上讲也就是虚拟内存上是连续的的空间，而在物理内存上就不一定了。

对于大多数垃圾回收期而言采用的都是标记-XXX算法（标记-清除、标记-复制、标记-整理）算法，为了记录标记的对象以及 GC 进度，需要使用数据结构进行保存。这些数据结构的确切大小和性质随实现的不同而变化，但是基本上与堆的大小成正比。
...
所谓的加载类就是将 .class 文件从磁盘、网络或者任何形式的存储器中加载到内存（方法区）。但加载过程并不是百分之百的，只有方法区中未找到此类的字符引用、或者此类还没有被加载到内从当中，才会执行。
...
但如果内存是杂乱的，就没有办法通过一个指针的方式简单表示了。此只能维护一个状态表存储那些内存已经被使用那些内存是空闲的，此方式叫做空闲列表。
