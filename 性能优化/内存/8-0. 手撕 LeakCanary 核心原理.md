...
```kotlin
  fun install(application: Application) {
    ...
    // 注册 Activity 生命周期回调监听
    ActivityDestroyWatcher.install(application, objectWatcher, configProvider)
    // 注册 Fragment 生命周期回调监听
...
==对于根本原因我的猜测是为了隔离与分层== 「 leakcanary-object-watcher-android」作为底层实现他是稳定的，他并不依赖上层可变的实现，他仅需要一个固定的实现类 `leakcanary.internal.InternalLeakCanary`   即可。放大一点我认为这种结构体现了 OOP 6大原则中的：

1. 单一职责原则：「 leakcanary-object-watcher-android」仅负责 watcher 对象销毁时机然后判定是否存在内存泄漏的可能。
2. 迪米勒原则（最少知识原则）：「 leakcanary-object-watcher-android」 模块仅仅了解和他需要的的 `leakcanary.internal.InternalLeakCanary` ，具体谁是现实、如何实现它都不管。

当然上面的描述有着生搬硬套的迹象，但是这并不妨碍我们学习这种写法的优点：==在底层逻辑并不关心上层逻辑的具体实现的同时，也解决了底层模块依赖上层模块的引用关系这大大降低了模块间耦合。==
...
==试验证明了 KeyedWeakReference 上面的注释是对的：弱引用一旦变得弱可达，就会立即入队。这将在 finalization 或者 GC 之前发生。但是如果 finalize 中自我救赎成功会自动出队，但弱引用还是为 null ，不会重新赋值==
