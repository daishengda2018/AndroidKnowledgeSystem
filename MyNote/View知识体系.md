...
* onMeasure 与 measure() 、onDraw 与 draw 的区别
  ...
  onXX 方法是用户调度的，而 measure、draw 才是真正做事情的

* 为什么不把对于尺寸的要求直接交个子 View 而是要交给父 View 呢？

  因为有些场景，尺寸需要父 View 进行规划，例如 LinearLayout 的子 View 全部设置了 weight 
...
`getLocationInWindow()` ：获取的是一个控件在其父控件的坐标位置
`getLocationOnScreent()`: 获取的是控件在屏幕上的坐标位置
getLocationOnScreen
`getLocationInWindow() `是以B为原点的C的坐标。
`getLocationOnScreen` 以A为原点，包括了状态栏的高度
 这两个方法在Activity的onCreate中使用获取的坐标永远是0，要等UI控件都加载完成之后才能获取。在`onWindowFocusChanged()` 中获取最好。因为在生命周期：onCreate、onStart、onResume中真正的View都没有可见。
