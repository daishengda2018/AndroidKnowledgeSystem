...
> 设计模式是写代码的招式，这些招式透露的核心思想都是一个：抵御变化，提高复用性。所以寻找并分离稳定与变化的部分，并变化替换为稳定尤为重要。理解什么时候、什么地点使用合适的招式，比理解它们本身更为重要。这正是厚积薄发、无招胜有招的境界。但是一开始的时候不要过度设计，当变化第一次发生的时候在运用它们化解变化才是最好的时机。
>
> 重构相当于一块磨刀石，目的是让代码不断使用接近设计模式，让代码更简单、更稳定、复用性和拓展性更好。
...
![image-20200514115152635](images/「Design」Design pattern/image-20200514115152635.png)

![image-20200514115108878](images/「Design」Design pattern/image-20200514115108878.png)
